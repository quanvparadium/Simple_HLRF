...
class ActorNetwork(nn.Module):
    def __init__(self, n_actions, input_dims, alpha, fc1_dims=256, fc2_dims=256, ckpt='weights/ppo'):
        super(ActorNetwork, self).__init__()

        self.checkpoint_file = os.path.join(ckpt, 'actor_torch_ppo')
    ...
    def forward(self, state):
        ...
        dist = Categorical(dist) # dist.sample(), get random value from {0, 1, 2, ..., K-1} with probability array
...
class CriticNetwork(nn.Module):
    def __init__(self, input_dims, alpha, fc1_dims=256, fc2_dims=256, ckpt_dir='weights/ppo'): 
        ...
        for _ in range(self.n_epochs):
            ...
            for t in range(len(reward_arr) - 1):
                ...
                for k in range(t, len(reward_arr) - 1):
                    a_t += discount * (reward_arr[k] + self.gae_lambda * values[k+1] * (1-int(dones_arr[k])) - values[k])
                    
                    discount = discount * (self.gamma*self.gae_lambda)
                advantage[t] = a_t

            # Convert to tensor in order to input model
            ...
            for batch in batches:
                ...
                old_probs = torch.tensor(old_prob_arr[batch], dtype=torch.float32).to(self.actor.device)
